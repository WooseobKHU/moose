...
ComputeSteinbachApelStrain::ComputeSteinbachApelStrain(const InputParameters & parameters)
  ...
  _elasticity_tensor_name(_base_B + "elasticity_tensor"),
  _elasticity_tensor(getMaterialPropertyByName<RankFourTensor>(_elasticity_tensor_name)),
  _compliance_tensor_name(_base_A + "compliance_tensor"),
  _compliance_tensor(getMaterialPropertyByName<RankFourTensor>(_compliance_tensor_name)),
...
ComputeSteinbachApelStrain::computeQpProperties()
{
  ...
  _ref[_qp] = _h_eta[_qp] * _identity[_qp] + (1.0 - _h_eta[_qp]) * _compliance_tensor[_qp] * _elasticity_tensor[_qp];

  _ref_inv[_qp] = _ref[_qp].invSymm();

  _mechanical_strain_b[_qp] = _ref_inv[_qp] * _global_mechanical_strain[_qp];
  _mechanical_strain_a[_qp] = _compliance_tensor[_qp] * _elasticity_tensor[_qp] * _mechanical_strain_b[_qp];
}
